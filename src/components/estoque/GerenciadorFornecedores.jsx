...
export default function GerenciadorFornecedores() {
    ...
    return (
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map(i => (
          <Star
            key={i}
            className={`w-3 h-3 ${i <= avaliacao ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
          />
        ))}
      </div>
    );
  ...
  if (isLoading) {
    return <div className="p-8">Carregando fornecedores...</div>;
  }
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {fornecedoresAtivos.length} fornecedores ativos
              </p>
            </div>
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        {fornecedor.nome_fantasia && (
                          <p className="text-xs text-gray-500">{fornecedor.nome_fantasia}</p>
                        )}
                      </div>
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEditar(fornecedor)}>
                          <Edit className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDeletar(fornecedor)} className="text-red-600">
                      ...
                      <div className="flex gap-2 mt-3">
                        {fornecedor.prazo_entrega_dias && (
                          <Badge variant="outline" className="text-xs">
                            {fornecedor.prazo_entrega_dias} dias entrega
                          </Badge>
                        )}
                        {fornecedor.avaliacao && (
                          <div className="flex items-center gap-1">
                            {getAvaliacaoStars(fornecedor.avaliacao)}
                          </div>
                        )}
                      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setDados({...dados, contato_responsavel: e.target.value})}
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                min="1"
                max="5"
                value={dados.avaliacao}
                onChange={(e) => setDados({...dados, avaliacao: e.target.value})}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                type="number"
                value={dados.prazo_entrega_dias}
                onChange={(e) => setDados({...dados, prazo_entrega_dias: e.target.value})}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setDados({...dados, pedido_minimo: e.target.value})}
                placeholder="0.00"
              />
            </div>
          </div>
