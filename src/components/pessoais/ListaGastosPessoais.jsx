...
const categoriaColors = {
  ...
  telefone: "bg-indigo-100 text-indigo-800",
  saude: "bg-pink-100 text-pink-800",
  educacao: "bg-amber-100 text-amber-800",
  lazer: "bg-orange-100 text-orange-800",
  transporte: "bg-slate-100 text-slate-800",
  roupas: "bg-rose-100 text-rose-800",
...
export default function ListaGastosPessoais({ gastos, loading, onEditar, onDeletar }) {
  ...
  if (loading) {
    return (
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle>Seus Gastos Pessoais</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Seus Gastos Pessoais ({gastos.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {gastos.map((gasto) => (
                  <TableRow key={gasto.id} className="hover:bg-gray-50">
                    ...
                    <TableCell>
                      <Badge variant="secondary" className={categoriaColors[gasto.categoria]}>
                        {categoriaLabels[gasto.categoria]}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      {gasto.metodo_pagamento ? (
                        <Badge variant="outline">
                          {metodoPagamentoLabels[gasto.metodo_pagamento]}
                        </Badge>
                      ) : '-'}
                    </TableCell>
                    <TableCell>
                ...
                {gastos.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                      Nenhum gasto pessoal registrado ainda.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
